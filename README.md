...
Note: For `Access > Establish Access - Entra`, MAAD-FV now supports either a saved Microsoft Graph token or manual username entry followed by interactive/device-code sign-in. Saved password credentials remain available for other services, but they are no longer shown as selectable Entra sign-in options.
